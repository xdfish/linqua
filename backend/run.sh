...
then
    ...
    echo "[] Setting up non persistent database (mongodb) via Docker"
    db () (
        trap - INT
    ...
    db
    docker stop linqua-db
    docker rm linqua-db
elif test "$1" = "prod"
then
    echo "LINQUA PRODUCTION MODE"
    echo "[] Setting up persistent database (mongodb) via Docker"
    if [ ! "$(docker ps -a | grep linqua-db)" ]
    then
         docker run -d -e MONGO_INITDB_ROOT_USERNAME='linqua_db_admin' -e MONGO_INITDB_ROOT_PASSWORD='dev_password' -v $SCRIPTPATH/server/database:/data/db -p 27017:27017 --name linqua-db mongo:latest
    fi    
